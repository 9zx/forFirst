...
<script type="text/javascript">
	$(function(){
		$("#chargeone_btn").click(function(){
			alert($("#chargeone_txt").val())
			alert($("#chargeone_sel").val())
			$.get("saveChargeOne.do",{charge_Code:$("#chargeone_sel").val(),charge:$("#chargeone_txt").val()},function(data){
				alert(data);
			});
		})
		

		$("#rulecharge_btn").click(function(){
			var arr = [];
			$("input:checkbox:checked").each(function () {
				 arr.push($(this).val()); 
			});
			alert(arr);
			$.get("saveChargeRule.do",{charge_Code:arr},function(data){
				alert(data);
			});			
		})
	});
...
<h2>收费细项</h2>
    <form method="post" action="saveChargeOne.do">
      <p>收费项目：<select size="1" name="charge_Code" id="chargeone_sel">
        <option value="A">开户费</option>
        <option value="B">漫游费</option>
        <option value="C">押金</option>
        <option value="F">入网费</option>
        <option value="E" >选号费</option>
      </select></p>
      <p>收费金额：<input type="text" name="charge" size="20" id="chargeone_txt"></p>
      <p><input type="button" value="提交" id="chargeone_btn" name="button" ><input type="reset" value="全部重写" name="B2"></p>
    </form>
    <hr>
    <h2>业务收费</h2>
    <form method="POST" action="--WEBBOT-SELF--">
      ...
      <p>收费项目：<input type="checkbox" id="rulecharge_Code" value="A">开户费 
      <input type="checkbox" id="rulecharge_Code" value="B" checked>漫游费 <input type="checkbox" id="rulecharge_Code" value="C" >押金 
      <input type="checkbox" id="rulecharge_Code" value="D">入网费 <input type="checkbox" name="rulecharge_Code"  value="E">选号费</p>
      <p><input type="button" value="提交" id="rulecharge_btn" name="button"><input type="reset" value="全部重写" name="B2"></p>
    </form>   
